 ...
 */
BaseController.prototype.action = function(actionName, fns) {
  // action('action_name', ...)
  if (fns) {
    // action('action_name', fn1, fn2, fn3)
    if (!(fns instanceof Array)) {
      // convert arguments object to array and discard first arguments
      fns = Array.prototype.slice.call(arguments, 1);
    }
    return this.addAction(actionName, fns);
  }
  ...
  if (action) {
    ...
    //console.log('/stacking filters');
    // actions is an array of functions
    this.callStack_ = this.callStack_.concat(action);
...
BaseController.prototype.addAction = function (actionName, fns) {
  this.actions[actionName] = fns;
}
